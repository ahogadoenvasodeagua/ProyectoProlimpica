...
def index(request):
    listatipoproduct = Producto.objects.all().order_by('nombres')
...
def guardar(request):
    ...
    if request.method == 'POST':
        if formulario_cliente.is_valid() :
            ...
            cliente.precio = datos_cliente.get('precio')
            cliente.tipoProduct = datos_cliente.get('tipoProduct')
            cliente.marca = datos_cliente.get('Marcas')
            cliente.fechaCaducidad = datos_cliente.get('fechaCaducidad')
